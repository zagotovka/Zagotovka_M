...
#define GSM &huart2
#define myDEBUG &huart3

/////////////////////////////////////// ИНФОРМАЦИОННЫЕ ДЕФАЙНЫ
////////////////////////////////////////////////
#define ATCPAS                                                                 \
  "AT+CPAS" // проверка статуса модема, ответ — (+CPAS: 0), (0 — готов к работе,
            // 2 — неизвестно, 3 — входящий звонок, 4 — в режиме соединения)
#define ATCREG                                                                 \
  "AT+CREG?" // регистрация в сети, интересен второй параметр в ответе - (+CREG:
             // 0,1), 0 - не зарегистрирован в сети, 1 - зерегистрированно, 2 -
             // не зарегистрированно идет поиск сети, 3 - регистрация отклонена,
             // 4 - неизвестно, 5 - зарегистрированно роуминг
...
#define ATCSQ                                                                  \
  "AT+CSQ" // уровень сигнала. Может быть в диапазоне 0..31, чем больше тем
           // лучше. 99 значит отсутствие сигнала.
...
#define ATCCLKK                                                                \
  "AT+CCLK=\"15/11/24,14:03:00+00\"" // установить дату/время -
                                     // yy/mm/dd,hh:mm:ss+zz
#define ATIPRS "AT+IPR=57600" // установить скорость порта
#define ATS                                                                    \
  "ATS0=0" // поднимать трубку только "вручную", ATS0=2 - поднимать трубку после
           // двух гудков, ATS0=3 - псле трёх и т.д.
...
/* Инициализация модуля SIM800L (вызывается из StartSIM800LTask) */
void init_sim800l_module(void);
/* Обработка входящих данных от SIM800L (вызывается в цикле StartSIM800LTask) */
