...
/******************** Zerg ************************/
bool ip_mac_printed = false;// Глобальная переменная для отслеживания, был ли уже выведен IP и MAC
// Новая функция для вывода IP и MAC
static void print_ip_and_mac(struct mg_tcpip_if *ifp) {
    if (!ip_mac_printed) {
        char ip[16], mac[18];
        // Преобразование IP адреса в строку
        snprintf(ip, sizeof(ip), "%lu.%lu.%lu.%lu",
                 (unsigned long)(ifp->ip & 255),
                 (unsigned long)((ifp->ip >> 8) & 255),
                 (unsigned long)((ifp->ip >> 16) & 255),
                 (unsigned long)((ifp->ip >> 24) & 255));
...
static void onstatechange(struct mg_tcpip_if *ifp) {
  if (ifp->state == MG_TCPIP_STATE_READY) {
    ...
    arp_ask(ifp, ifp->gw);
    print_ip_and_mac(ifp);  // Вызов моей функции
