...
const char *s_json_header = "Content-Type: application/json\r\n"
...
void fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data) {
  ...
  switch (ev) {
  case MG_EV_OPEN:
    MG_INFO(("%lu Connection created (TLS: %d, fn_data: %p)", c->id, c->is_tls,
             fn_data));
    break;

  case MG_EV_ACCEPT: {
    unsigned int local_port = mg_ntohs(c->loc.port);
    MG_INFO(("Accept: local %M remote %M", mg_print_ip_port, &c->loc,
    ...
    if (local_port == atoi(HTTP_PORT)) {
      ...
      if (fn_data != NULL && SetSettings.usehttps == 1) {
        // Локальные буферы для сертификатов и ключей (убираем static, чтобы
        // избежать проблем в многопоточной среде)
        ...
        // Проверка корректности формата данных (опционально, для повышения
        // надежности)
        ...
        // Прямое сравнение байт за байтом данных (cert_buffer и ) с теми,
        // которые работают (TLS_CERT и TLS_KEY), чтобы убедиться, что они
        // идентичны
        //                uint8_t mis_sert = 0;
        //                uint8_t mis_key = 0;
        //                for (size_t i = 0; i < strlen(TLS_CERT) && i <
        //                strlen(cert_buffer); i++) {
        //                    if (TLS_CERT[i] != cert_buffer[i]) {
        //                        printf("mis_sert at position %u:
        //                        TLS_CERT[%u]='%c' (0x%02X), mis_sert[%u]='%c'
        //                        (0x%02X)\n",
        //                               i, i, TLS_CERT[i], (unsigned
        //                               char)TLS_CERT[i], i, cert_buffer[i],
        //                               (unsigned char)cert_buffer[i]);
        //                        mis_sert++;
        //                        if (mis_sert >= 254) break;  // Ограничиваем
        //                        количество расхождений в выводе
        //                    }
        //                }
        //                for (size_t i = 0; i < strlen(TLS_KEY) && i <
        //                strlen(key_buffer); i++) {
        //                    if (TLS_KEY[i] != key_buffer[i]) {
        //                        printf("mis_key at position %u:
        //                        TLS_KEY[%u]='%c' (0x%02X), mis_key[%u]='%c'
        //                        (0x%02X)\n",
        //                               i, i, TLS_KEY[i], (unsigned
        //                               char)TLS_KEY[i], i, key_buffer[i],
        //                               (unsigned char)key_buffer[i]);
        //                        mis_key++;
        //                        if (mis_key >= 254) break;  // Ограничиваем
        //                        количество расхождений в выводе
        //                    }
        //                }
  ...
  case MG_EV_HTTP_MSG: {
    ...
    MG_INFO(("%lu Received HTTP/HTTPS message (TLS: %d, URI: %.*s)", c->id,
             c->is_tls, (int)hm->uri.len, hm->uri.buf));

    if (c->is_tls) {
      MG_INFO(("%lu HTTPS message received: %.*s", c->id, (int)hm->uri.len,
               hm->uri.buf));
    } else {
      MG_INFO(("%lu HTTP message received: %.*s", c->id, (int)hm->uri.len,
               hm->uri.buf));
    }
    ...
    MG_INFO(("%lu Checking URI: %.*s (len: %d)", c->id, (int)uri.len, uri.buf,
             (int)uri.len));
    ...
    if (uri.len == 1 && strncmp(uri.buf, "/", uri.len) == 0) {
      ...
      bool is_connection_close =
          conn_value && mg_vcmp(conn_value, "close") == 0;

      if (is_connection_close) {
        // Если клиент запросил Connection: close, используем кастомную отправку
        // ответа
        ...
        MG_INFO(
            ("%lu Using packed filesystem root_dir: %s", c->id, opts.root_dir));
#endif
        ...
        // Если клиент не запросил Connection: close, используем
        // mg_http_serve_dir
        char extra_headers[256];
        snprintf(extra_headers, sizeof(extra_headers),
      ...
      if (mg_match(hm->uri, mg_str("/api/#"), NULL) && u == NULL) {
        MG_ERROR(("%lu Unauthorized API request: %.*s", c->id, (int)hm->uri.len,
                  hm->uri.buf));
        const char *conn_hdr = get_connection_header(hm);
        ...
        if (second_slash) {
          ...
          if (token_len > 0 && token_len == strlen(SetSettings.token) &&
            ...
            size_t command_len =
                end ? (size_t)(end - command) : strlen(command);
            ...
            if (command_len > 0) {
              ...
              if (strncmp(cmd_after_token, "switch", 6) == 0 ||
                ...
                MG_INFO(("%lu Processing API command with token: %.*s", c->id,
                         (int)command_len, command));
                // Передаём оригинальный hm с полным URI (/api/Zerg/pwm),
                // чтобы api_handler мог найти "/api/" и корректно распарсить
                // токен
                api_handler(c, hm);
              } else {
                MG_ERROR(("%lu Invalid command format: %.*s", c->id,
                          (int)(command_len - token_len - 1), cmd_after_token));
            ...
            MG_ERROR(
                ("%lu Invalid token: %.*s", c->id, (int)token_len, api_path));
            mg_http_reply(c, 401, NULL, "Invalid token\n");
          }
        } else {
          MG_ERROR(("%lu Invalid API path format: %.*s", c->id,
                    (int)hm->uri.len, hm->uri.buf));
          mg_http_reply(c, 400, NULL, "Invalid API path format\n");
        }
      } else {
        // Обработка статических страниц для всех остальных запросов
        MG_INFO(("%lu Did not match any API pattern, serving static content",
                 c->id));
        struct mg_http_serve_opts opts;
        memset(&opts, 0, sizeof(opts));
#if MG_ARCH == MG_ARCH_UNIX || MG_ARCH == MG_ARCH_WIN32
        opts.root_dir = "web_root";
        MG_INFO(("%lu Using filesystem root_dir: %s for static content", c->id,
                 opts.root_dir));
#else
        opts.root_dir = "/web_root";
        opts.fs = &mg_fs_packed;
        MG_INFO(("%lu Using packed filesystem root_dir: %s for static content",
                 c->id, opts.root_dir));
#endif
        mg_http_serve_dir(c, ev_data, &opts);
  ...
  case MG_EV_POLL:
    ...
    // Здесь можно добавить проверку таймаутов или обновление состояния, если
    // требуется
    break;
...
static void fn_mqtt(struct mg_connection *c, int ev, void *ev_data,
  ...
  if (ev == MG_EV_OPEN) {
    ...
    struct mg_str pubt = mg_str(get_mqtt_topic()),
                  data = mg_str("Hello from stm32!");
...
void timer_fn_mqtt(void *arg) {
  ...
  struct mg_mqtt_opts opts = {.clean = true,
                              ...
                              .message = mg_str("bye")};
  if (s_conn == NULL)
    s_conn = mg_mqtt_connect(mgr, get_mqtt_url(), &opts,
                             (mg_event_handler_t)fn_mqtt, NULL);
}
...
//   timer_sntp_fn, mgr); mg_timer_add(mgr, 1000, MG_TIMER_REPEAT |
//   MG_TIMER_RUN_NOW, timer_fn_mqtt, mgr);
// }
void web_init(struct mg_mgr *mgr) {
  ...
  // Запуск HTTP-сервера
  struct mg_connection *http_conn =
      mg_http_listen(mgr, http_url, (mg_event_handler_t)fn, NULL);
  if (http_conn) {
    MG_INFO(
        ("HTTP server started on %s, conn=%p", http_url, (void *)http_conn));
  } else {
    MG_ERROR(("Failed to start HTTP server on %s", http_url));
  }

  // Запуск HTTPS-сервера с маркером HTTPS_MARKER
  struct mg_connection *https_conn =
      mg_http_listen(mgr, https_url, (mg_event_handler_t)fn, HTTPS_MARKER);
  if (https_conn) {
    MG_INFO(
        ("HTTPS server started on %s, conn=%p", https_url, (void *)https_conn));
  } else {
  ...
  struct mg_connection *mqtt_conn =
      mg_mqtt_listen(mgr, mqtt_url, (mg_event_handler_t)fn, NULL);
  if (mqtt_conn) {
    MG_INFO(
        ("MQTT server started on %s, conn=%p", mqtt_url, (void *)mqtt_conn));
  } else {
  ...
  mg_timer_add(mgr, 10 * 1000, MG_TIMER_RUN_NOW | MG_TIMER_REPEAT,
               timer_sntp_fn, mgr);
  mg_timer_add(mgr, 1000, MG_TIMER_REPEAT | MG_TIMER_RUN_NOW, timer_fn_mqtt,
               mgr);
}
/*********************************** From Zagotovka
 * ****************************************************/
...
void setup_mqtt(struct mg_mgr *mgr, struct mg_tcpip_if *mif) {
  ...
  if (SetSettings.check_mqtt) {
    ...
    mg_timer_add(mgr, 3000, MG_TIMER_REPEAT | MG_TIMER_RUN_NOW, timer_fn_mqtt,
                 mgr);
    *flagmqtt = true; // Устанавливаем флаг, что инициализация выполнена
  } else {
    MG_INFO(("MQTT is disabled in settings"));
    *flagmqtt =
        true; // Устанавливаем флаг, чтобы не пытаться инициализировать снова
  }
}
