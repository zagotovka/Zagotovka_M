...
#define DEBOUNCE_DELAY                                                         \
  10 // EncoderTask & SecurityTask (ms).  Если у вас оптический или магнитный
     // энкодер — можно смело ставить 3–5 мс.
...
extern uint8_t
    RxByte; // Буфер для приёма одного байта по UART (определён в gsm.c)
...
/**************************** GSM (defined in gsm.c)
 * ***************************************/
extern char dtmf_buf[DTMF_BUF_SIZE]; // Buffer to store DTMF digits
extern uint8_t dtmf_idx;             // Index for DTMF buffer
extern char vldpins[240];            // Буфер для валидных пинов
extern char invpins[256];            // Буфер для невалидных пинов
extern int validcnt;
extern int invldcnt;
extern char buf[GSM_RX_BUFFER_SIZE];
extern char str2[GSM_RX_BUFFER_SIZE];
/*************************** END GSM ************************************/
/* USER CODE END PV */
...
/* Отправка сообщения в очередь MQTT только если MQTT включён в настройках.
 * Возвращает pdPASS если отправлено, pdFAIL если MQTT отключён или очередь
 * полна. */
BaseType_t mqtt_enqueue(const MqttMessage_t *msg) {
  if (SetSettings.check_mqtt != 1 || SetSettings.txmqttop[0] == '\0') {
    return pdFAIL; // MQTT отключён — тихо игнорируем
  }
  return xQueueSend(mqttQueueHandle, msg, 0);
}
...
void pwm_event_handler(Button *handle) {
  PressEvent event = get_button_event(handle);
  uint8_t btn_id = handle->button_id;

  if (event == LONG_PRESS_HOLD || event == SINGLE_CLICK) {
    for (uint8_t a = 0; a < NUMPINLINKS; a++) {
      if (PinsLinks[a].idin == btn_id) {
        uint8_t idpwm = PinsLinks[a].idout;
        if (PinsConf[idpwm].topin == 5) {
          if (PinsConf[btn_id].on == 1) {
            if (PinsConf[idpwm].dvalue < 100)
              PinsConf[idpwm].dvalue++;
            else
              PinsConf[btn_id].on = 0;
          } else {
            if (PinsConf[idpwm].dvalue > 0)
              PinsConf[idpwm].dvalue--;
            else
              PinsConf[btn_id].on = 1;
          }
          uint32_t ccr =
              PercentToCCR(PinsConf[idpwm].dvalue, PinsConf[idpwm].pwmmax);
          __HAL_TIM_SET_COMPARE(&htim[idpwm], PinsInfo[idpwm].tim_channel, ccr);
        }
      }
    }
  ...
  for (;;) {
    if (xQueueReceive(outputQueueHandle, &data_pin, portMAX_DELAY) == pdTRUE) {
      if (data_pin.id >= 0 && data_pin.id < NUMPIN) {
        ...
        case 0:                                   // OFF
          if (PinsConf[data_pin.id].topin == 5) { // Если ШИМ
            PinsConf[data_pin.id].dvalue = 0;
            uint32_t ccr = PercentToCCR(0, PinsConf[data_pin.id].pwmmax);
            __HAL_TIM_SET_COMPARE(&htim[data_pin.id],
                                  PinsInfo[data_pin.id].tim_channel, ccr);
          } else {
        ...
        case 1:                                   // ON
          if (PinsConf[data_pin.id].topin == 5) { // Если ШИМ
            PinsConf[data_pin.id].dvalue = 100;
            uint32_t ccr = PercentToCCR(100, PinsConf[data_pin.id].pwmmax);
            __HAL_TIM_SET_COMPARE(&htim[data_pin.id],
                                  PinsInfo[data_pin.id].tim_channel, ccr);
          } else {
            HAL_GPIO_WritePin(PinsInfo[data_pin.id].gpio_name,
          ...
          break;
        case 2: // TOGGLE
          if (PinsConf[data_pin.id].topin ==
              5) { // Для ШИМ ТОГГЛ — это инверсия 0/100%
            PinsConf[data_pin.id].dvalue =
                (PinsConf[data_pin.id].dvalue > 0) ? 0 : 100;
            uint32_t ccr = PercentToCCR(PinsConf[data_pin.id].dvalue,
                                        PinsConf[data_pin.id].pwmmax);
            __HAL_TIM_SET_COMPARE(&htim[data_pin.id],
                                  PinsInfo[data_pin.id].tim_channel, ccr);
          } else {
...
void StartEncoderTask(void *argument) {
  ...
  // Инициализируем prev_A реальными состояниями GPIO, чтобы не возникал
  // ложный нарастающий фронт на первой итерации (если пин уже в HIGH)
  for (uint8_t _id = 0; _id < NUMPIN; _id++) {
    if (PinsConf[_id].topin == 8) {
      prev_A[_id] =
          HAL_GPIO_ReadPin(PinsInfo[_id].gpio_name, PinsInfo[_id].hal_pin);
    }
  }
  uint32_t lastEncoderChange = 0;   // Время последнего вращения любого энкодера
  bool needSave = false;            // Флаг: нужно ли сохранить на флешку
  uint32_t lastPulse[NUMPIN] = {0}; // Время последнего щелчка энкодера
#define ENC_FAST_THRESHOLD_MS 150   // Если между щелчками < 150 мс — быстро
#define ENC_FAST_STEP 5             // Шаг при быстром вращении
#define ENC_SLOW_STEP 1             // Шаг при медленном вращении
  ...
  for (;;) {
    ...
    for (uint8_t id = 0; id < NUMPIN; id++) {
      ...
      // ── Дебаунс ───────────────────────────────────────────────────────────
      if ((millis - pinTimes[id]) < DEBOUNCE_DELAY)
        continue;
      ...
      if (stateA == 1 && prev_A[id] == 0) {
        ...
        const int8_t step =
            (interval < ENC_FAST_THRESHOLD_MS) ? ENC_FAST_STEP : ENC_SLOW_STEP;
...
void StartMqttTask(void *argument) {
  ...
  for (;;) {
    ...
    if (status == pdPASS) {
      ...
      case 1: // DEVICE
        if (SetSettings.txmqttop[0] != '\0' && SetSettings.check_mqtt == 1) {
          if (check_mqtt_connection(s_conn)) {
            ...
            // MQTT broker not connected — throttled message below
          }
        }
        break;
      case 2: // Switch
      ...
      // Если брокер недоступен — печатаем не чаще 1 раза в 10 секунд
      if (!check_mqtt_connection(s_conn)) {
        static uint32_t t_last_notconn = 0;
        uint32_t tnow = HAL_GetTick();
        if (tnow - t_last_notconn >= 10000) {
          t_last_notconn = tnow;
    ...
    // Печатаем ошибку не чаще 1 раза в 10 секунд если MQTT включён но брокер
    // недоступен
    if (SetSettings.check_mqtt == 1 && !check_mqtt_connection(s_conn)) {
      static uint32_t t_mqtt_offline = 0;
      uint32_t tnow2 = HAL_GetTick();
      if (tnow2 - t_mqtt_offline >= 10000) {
        t_mqtt_offline = tnow2;
        printf("MQTT: waiting for broker connection...\r\n");
      }
    }
    //		if(1){
