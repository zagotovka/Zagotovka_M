...
uint32_t GetTimerClock(TIM_TypeDef *tim) {
  if (tim == TIM1 || tim == TIM8 || tim == TIM9 || tim == TIM10 ||
      tim == TIM11) {
    return 216000000;
  }
  return 108000000;
...
void CalcTimerParams(TIM_TypeDef *tim, uint32_t freq_millihz, uint32_t *psc_out,
                     uint32_t *arr_out) {
  if (freq_millihz < 50)
    freq_millihz = 50;
  uint32_t timer_clock = GetTimerClock(tim);
  uint64_t total = ((uint64_t)timer_clock * 1000ULL) / freq_millihz;
  uint64_t arr_max = (tim == TIM2 || tim == TIM5) ? 0xFFFFFFFFULL : 0xFFFFULL;

  if (total <= arr_max + 1ULL) {
    *psc_out = 0;
    *arr_out = (uint32_t)(total - 1);
  } else {
    uint32_t psc = (uint32_t)(total / (arr_max + 1ULL));
    if (psc > 65535)
      psc = 65535;
    *psc_out = psc;
    *arr_out = (uint32_t)(total / (uint64_t)(psc + 1)) - 1;
  }
}
...
void SetCronConfig() {
  ...
  for (int i = 0; i < MAXSIZE; i++) {
    ...
    writeField("ptime", "%lu", dbCrontxt[i].ptime);
...
void GetPinConfig() {
  UINT bytesRead;
  char key[32] = {0}, value[128] = {0};
  int keyIndex = 0, valueIndex = 0, currentPin = 0;
  bool inObject = false, inQuotes = false, readingValue = false;
    ...
    if (currentChar == '}' || (currentChar == ',' && !inQuotes)) {
      if (keyIndex > 0 && valueIndex > 0) {
        ...
        if (strcmp(key, "topin") == 0)
          ...
          PinsConf[currentPin].pwm = strtoul(value, NULL, 10);
        else if (strcmp(key, "pwmmax") == 0)
          PinsConf[currentPin].pwmmax = strtoul(value, NULL, 10);
          ...
          PinsConf[currentPin].dvalue = (uint8_t)atoi(value);
          ...
          strncpy(PinsConf[currentPin].encbpin, value,
...
void SetPinConfig() {
  ...
  for (int i = 0; i < NUMPIN; i++) {
    ...
    snprintf(buffer, sizeof(buffer), ",\"pwm\":%lu", PinsConf[i].pwm);
    fresult = f_write(&USBHFile, buffer, strlen(buffer), &Byteswritten);
    if (fresult != FR_OK) {
...
void InitPin() {
  ...
  for (i = 0; i < NUMPIN; i++) {
    ...
    if (PinsConf[i].topin == 2 || PinsConf[i].topin == 4) {
      ...
      // Восстанавливаем состояние пина при включении
      GPIO_PinState initialState =
          (PinsConf[i].onoff == 1) ? GPIO_PIN_SET : GPIO_PIN_RESET;
      ...
      CalcTimerParams(PinsInfo[i].tim, PinsConf[i].pwm, &psc_val, &arr_val);
      PinsConf[i].pwmmax = arr_val;
      htim[i].Instance = PinsInfo[i].tim;
      htim[i].Init.Prescaler = psc_val;
      htim[i].Init.CounterMode = TIM_COUNTERMODE_UP;
      htim[i].Init.Period = arr_val;
      ...
      // Если Power On Restore выключен — сбрасываем dvalue в 0 при старте
      if (PinsConf[i].ponr == 0) {
        PinsConf[i].dvalue = 0;
      }
      sConfigOC.Pulse = PercentToCCR(PinsConf[i].dvalue, PinsConf[i].pwmmax);
      ...
      case 0:
        ...
        GPIO_InitStruct.Pull = GPIO_PULLDOWN;
        PinsConf[i].state = 0;
        PinsConf[i].prvstate = 0;
        break;
      case 1:
        GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
        GPIO_InitStruct.Pull = GPIO_PULLDOWN;
        PinsConf[i].state = 1;
        PinsConf[i].prvstate = 1;
        break;
      case 2:
        GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
        GPIO_InitStruct.Pull = GPIO_PULLDOWN;
